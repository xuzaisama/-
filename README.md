...
系统默认启用本地声纹识别。首次运行时会引导用户录入 5 句固定文本，并在项目根目录生成 `voiceprint_profile.json`。
...
这部分逻辑位于 [robot_auditory/tts.py](robot_auditory/tts.py) 和 [robot_auditory/listener.py](robot_auditory/listener.py)。
...
- [robot_auditory/main.py](robot_auditory/main.py)：命令行入口，组织监听、声纹预热、指令闭环
- [robot_auditory/gui_app.py](robot_auditory/gui_app.py)：当前 GUI 主程序，集成 3D 场景、语音车控、语音问答
- [robot_auditory/listener.py](robot_auditory/listener.py)：麦克风采集、Google 语音识别、声纹过滤接入
- [robot_auditory/config.py](robot_auditory/config.py)：系统参数、命令词表、距离角度速度解析
- [robot_auditory/controller.py](robot_auditory/controller.py)：基础控制闭环、动作执行、去重与播报联动
- [robot_auditory/voiceprint.py](robot_auditory/voiceprint.py)：声纹建模、存档、相似度校验
- [robot_auditory/enrollment.py](robot_auditory/enrollment.py)：首次录入与重录流程
- [robot_auditory/info_tools.py](robot_auditory/info_tools.py)：时间与天气实时工具
- [robot_auditory/tts.py](robot_auditory/tts.py)：文本转语音与监听保护
...
- 问答功能依赖千问兼容接口
- 首次使用 PaddleSpeech 声纹模型时会下载预训练权重到 `.cache/paddlespeech`
...
- [tests/test_config.py](tests/test_config.py)：指令关键词、距离角度速度、连续命令解析
- [tests/test_gui_motion.py](tests/test_gui_motion.py)：3D 小车运动学逻辑
- [tests/test_info_tools.py](tests/test_info_tools.py)：时间与天气工具层
- [tests/test_tts_guard.py](tests/test_tts_guard.py)：播报保护与回声屏蔽
- [tests/test_voiceprint.py](tests/test_voiceprint.py)：声纹录入与校验逻辑
...
- 将 [robot_auditory/controller.py](robot_auditory/controller.py) 中的默认动作处理替换为串口、ROS、Socket 或仿真平台接口，接入真实机器人。
